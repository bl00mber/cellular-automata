  ...
  getRulesForDropdown = () => {
    ...
    for (const key in rules) {
      ...
      items.push({section: category, label, id, onClick: () =>
        this.setState({ controlsChanged: true, ruleNumber, rule: rules['rule'+ruleNumber].slice() })})
    }
  ...
  render() {
    ...
    return(
      <div className="container">
        ...
        <Button className="random-rule-button" variant="outlined" onClick={() => this.setRandomRule()}>
            Random Rule
          </Button>
        ...
        {(ruleNumber == 30) && <div className="rule-text">
          The most interesting rule due to <a href="https://www.wolframscience.com/nks/notes-2-1--rule-30/">randomness</a> generated <a href="https://www.wolframscience.com/nks/notes-10-9--tests-of-randomness/">by it</a>. Patterns generated by cellular automatons can be found in nature, and for the rule 30 it is mollusk <a href="https://en.wikipedia.org/wiki/Conus_textile">Conus textile</a>.
        </div>}

        {(ruleNumber == 90) && <div className="rule-text">
          The fractal produced by this rule also known as the Sierpinski triangle was described by Sierpinski in 1915 and appearing in <a href="https://www.wolframscience.com/nks/p43--why-these-discoveries-were-not-made-before/">Italian art</a> from the 13th century. Coloring all odd numbers black and even numbers white in <a href="https://en.wikipedia.org/wiki/File:Pascal%27s_Triangle_with_odd_numbers_shaded.svg">Pascal's triangle</a> produces a <a href="https://en.wikipedia.org/wiki/File:Sierpinski_triangle.svg">Sierpinski triangle</a>.
        </div>}

        {(ruleNumber == 110) && <div className="rule-text">
          Rule 110 is <a href="https://wpmedia.wolfram.com/uploads/sites/13/2018/02/15-1-1.pdf">known to be Turing complete</a>. This implies that, in principle, <a href="https://arxiv.org/pdf/0906.3248.pdf">any calculation</a> or computer program <a href="https://dna.hamilton.ie/woods/download/NearyWoodsBCRI-04-06.pdf">can be simulated</a> using this automaton.
        </div>}

        {(ruleNumber == 184) && <div className="rule-text">
          <a href="https://en.wikipedia.org/wiki/Rule_184">Rule 184</a> can be used as a simple model for traffic flow in a single lane of a highway, and forms the basis for many cellular automaton models of traffic flow with greater sophistication. In this model, particles (representing vehicles) move in a single direction, stopping and starting depending on the cars in front of them. The number of particles remains unchanged throughout the simulation. Because of this application, Rule 184 is sometimes called the "traffic rule".
        </div>}
